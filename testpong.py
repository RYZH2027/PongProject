        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if single_player_button.collidepoint(event.pos):
                    global game_mode
                    game_mode = 'single'
                    score_selection_menu()  # Go to score selection menu
...
def score_selection_menu():
    """Score limit selection menu."""
    global score_limit
    while True:
        screen.fill(BLUE)

        # Create buttons for score limits
        score_10_button = create_button(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 - 50, 200, 50, "10 Point Limit", font, RED, WHITE)
        score_20_button = create_button(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 + 50, 200, 50, "20 Point Limit", font, RED, WHITE)
        back_button = create_button(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 + 150, 200, 50, "Back", font, RED, WHITE)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if score_10_button.collidepoint(event.pos):
                    score_limit = 10  # Set score limit to 10
                    reset_score()  # Reset score before starting the game
                    game_loop()  # Start the game
                if score_20_button.collidepoint(event.pos):
                    score_limit = 20  # Set score limit to 20
                    reset_score()  # Reset score before starting the game
                    game_loop()
...
def game_loop():
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    reset_score()  # Reset the score when ESC is pressed
        ...
        if player1_score >= score_limit or player2_score >= score_limit:
            ...
            reset_score()  # Reset scores before returning to the menu
